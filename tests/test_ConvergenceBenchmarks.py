...
def generic_convergence_test(
    ...
    assert not close_to_a_minimum(
        net.weights, global_minima, atol / 1000, rtol / 1000
...
def test_square_plus_2_converges(optimizer_type: Type[GenericPSO]) -> None:
    class SquarePlus2WeightsModule(torch.nn.Module):
        def __init__(self):
            super().__init__()
            self.weights = torch.nn.Parameter(torch.rand((1,)))
            self.global_minima = [torch.Tensor([-2.0])]
    ...
    return generic_convergence_test(
        optimizer_type=optimizer_type, net=SquarePlus2WeightsModule(), atol=1e-1, rtol=1e-1, max_iterations=3000
    )
...
class RastriginModule(torch.nn.Module):
    def __init__(self, num_dimensions, a=10.0):
        super().__init__()
...
@optimizer_tests(ignore=['ChaoticPSO', 'DolphinPodOptimizer', 'RingTopologyPSO'])
def test_ackley_function(optimizer_type: Type[GenericPSO]) -> None:
    class AckleyModule(torch.nn.Module):
        def __init__(self):
            super().__init__()
            self.weights = torch.nn.Parameter(torch.rand((2,)))
            self.global_minima = [torch.Tensor([0.0, 0.0])]
    ...
    return generic_convergence_test(
        optimizer_type=optimizer_type, net=AckleyModule(), atol=1e-1, rtol=1e-1, max_iterations=1000
    )
